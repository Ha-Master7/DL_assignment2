...
def rnn_backward(dh, cache):
    ...
    dx = torch.zeros(N, T, D, device=x.device, dtype=x.dtype)
    dWx = torch.zeros(D, H, device=x.device, dtype=x.dtype)
    dWh = torch.zeros(H, H, device=x.device, dtype=x.dtype)
    db = torch.zeros(H, device=x.device, dtype=x.dtype)

    dprev_h = torch.zeros(N, H, device=x.device, dtype=x.dtype)

    for t in reversed(range(T)):
        dtotal_h = dh[:, t, :] + dprev_h
        dx_t, dprev_h, dWx_t, dWh_t, db_t = rnn_step_backward(dtotal_h, caches[t])
        dx[:, t, :] = dx_t
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        # Project pooled image features to initial hidden state h0
        ...
        if self.cell_type == "rnn":
            ...
            self.attn_proj = nn.Conv2d(input_dim, hidden_dim, kernel_size=1)
    ...
    def forward(self, images, captions):
        ...
        if self.cell_type == "attn":
            # Attention LSTM uses spatial feature map directly
            A = self.attn_proj(features)
        else:
            # RNN / LSTM: global average pooling + projection
            features = features.mean(dim=(2, 3))  # (N, C)
            h0 = self.feature_proj(features)  # (N, H)

        # (2) Embed input captions
        word_vectors = self.word_embedding(captions_in)  # (N, T, W)

        # (3) Run recurrent model
        if self.cell_type == "attn":
            h = self.rnn(word_vectors, A)  # AttentionLSTM.forward(x, A)
        else:
            h = self.rnn(word_vectors, h0)  # RNN/LSTM.forward(x, h0)

        # (4) Compute vocabulary scores
    ...
    def sample(self, images, max_length=15):
        ...
        # (2) Start token
        prev_word = images.new(N).fill_(self._start).long()

        # (3) Sampling loop
        for t in range(max_length):
            # (3.1) Embed word
            word_vec = self.word_embedding(prev_word)
...
def dot_product_attention(prev_h, A):
    ...
    A_flat = A.view(N, H, D_a * D_a)  # (N, H, 16)

    # (2) Scaled dot-product
    scores = torch.bmm(
        prev_h.unsqueeze(1),  # (N, 1, H)
        A_flat  # (N, H, 16)
    ).squeeze(1) / (H ** 0.5)  # (N, 16)

    # (3) Softmax over spatial locations
    alpha = torch.softmax(scores, dim=1)  # (N, 16)

    # (4) Attention embedding
    attn = torch.bmm(
        A_flat,  # (N, H, 16)
        alpha.unsqueeze(2)  # (N, 16, 1)
    ).squeeze(2)  # (N, H)

    # (5) Reshape attention weights
    attn_weights = alpha.view(N, D_a, D_a)  # (N, 4, 4)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class AttentionLSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, A: torch.Tensor):
        ...
        # Initial hidden and cell states (given)
        h = h0  # (N, H)
        c = c0  # (N, H)

        # Loop over timesteps
        for t in range(T):
            # (1) Compute attention from previous hidden state
            attn, _ = dot_product_attention(h, A)  # (N, H)

            # (2) Attention LSTM step
            h, c = self.step_forward(x[:, t, :], h, c, attn)

            # (3) Store hidden state
            hn[:, t, :] = h
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
