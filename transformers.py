...
def prepocess_input_sequence(
    ...
    for tok in tokens:
        # case 1: special token (POSITIVE, NEGATIVE, add, subtract, BOS, EOS)
        if tok in spc_tokens:
            out.append(token_dict[tok])

        # case 2: number token → break into digits
        else:
            for digit in tok:
...
def scaled_dot_product_two_loop_single(
    ...
    for i in range(K):
        # compute scaled dot products for query i with all keys
        scores = torch.zeros(K, device=query.device)

        for j in range(K):
            scores[j] = torch.dot(query[i], key[j]) / torch.sqrt(torch.tensor(M, device=query.device))
...
def scaled_dot_product_two_loop_batch(
    ...
    for n in range(N):
        for i in range(K):
            # compute scaled dot products for query[n, i] with all keys in batch n
            scores = torch.zeros(K, device=query.device)

            for j in range(K):
                scores[j] = torch.dot(query[n, i], key[n, j]) / torch.sqrt(torch.tensor(M, device=query.device))
...
def scaled_dot_product_no_loop_batch(
    ...
    scores = torch.bmm(query, key.transpose(1, 2)) / torch.sqrt(
        torch.tensor(M, device=query.device)
    )
    if mask is not None:
        ##########################################################################
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.q = nn.Linear(dim_in, dim_q, bias=True)
        self.k = nn.Linear(dim_in, dim_q, bias=True)
        self.v = nn.Linear(dim_in, dim_v, bias=True)
    ...
    def forward(
        ...
        y, weights = scaled_dot_product_no_loop_batch(Q, K, V, mask)

        # store attention weights
        self.weights_softmax = weights
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        # 1. create list of SingleHeadAttention modules
        ...
        # 2. linear layer to map concatenated heads back to dim_in
        self.linear = nn.Linear(num_heads * dim_out, dim_in, bias=True)

        # weight initialization (same strategy as SelfAttention)
...
class LayerNormalization(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        # compute mean over embedding dimension
        mean = x.mean(dim=-1, keepdim=True)

        # compute variance (not using torch.std)
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        # Replace "pass" statement with your code
        # first linear layer
        self.linear1 = nn.Linear(inp_dim, hidden_dim_feedforward, bias=True)

        # second linear layer (back to inp_dim)
        self.linear2 = nn.Linear(hidden_dim_feedforward, inp_dim, bias=True)
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        # each head outputs emb_dim / num_heads
    ...
    def forward(self, x):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        # MultiHeadAttention (Q = K = V = x)
        attn_out = self.mha(x, x, x)

        # Residual + LayerNorm + Dropout
        x2 = self.dropout(self.ln1(x + attn_out))

        # FeedForward
        ff_out = self.ff(x2)

        # Residual + LayerNorm + Dropout
        y = self.dropout(self.ln2(x2 + ff_out))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
def get_subsequent_mask(seq):
    ...
    if seq.dim() == 2:
        ...
        # (K, M)
        K = seq.shape[0]
        N = K  # this matches the expected test behavior

    # upper triangular matrix (excluding diagonal)
    mask = torch.triu(
        torch.ones((K, K), dtype=torch.bool),
        diagonal=1
    )
...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        ##########################################################################
        # 1) Masked self-attention (decoder self interaction)
    ...
    def forward(
        ...
        # ---- 2) Cross-attention ----
        # Query comes from decoder path (out2); key/value come from encoder output (enc_inp)
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    # positions from 0 to K-1 scaled to [0, 1)
    base = torch.arange(K, dtype=torch.float32) / K  # shape (K,)

    # replicate across embedding dimension and add batch dimension
    y = base.unsqueeze(0).repeat(M, 1).transpose(0, 1).unsqueeze(0)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Create position indices: [0, 1, 2, ..., K-1]
    position = torch.arange(K, dtype=torch.float32).unsqueeze(1)  # shape (K, 1)
    
    # Create dimension indices: [0, 1, 2, ..., M-1]
    dim_idx = torch.arange(M, dtype=torch.float32)  # shape (M,)
    
    # Compute the division term: 10000^(2i/M) for all dimensions
    div_term = torch.pow(10000.0, (2 * (dim_idx // 2)) / M)  # shape (M,)
    
    # Compute angles: position / div_term
    angles = position / div_term  # shape (K, M) via broadcasting
    
    # Apply sin to even indices (0, 2, 4, ...)
    # Apply cos to odd indices (1, 3, 5, ...)
    y = torch.zeros(K, M)
    y[:, 0::2] = torch.sin(angles[:, 0::2])  # even dimensions
    y[:, 1::2] = torch.cos(angles[:, 1::2])  # odd dimensions
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        ##########################################################################
        # Pass question embeddings through encoder
        enc_out = self.encoder(q_emb_inp)
        
        # Create mask for target sequence (prevent attending to future tokens)
        mask = get_subsequent_mask(ans_b)
        
        # Pass answer embeddings and encoder output through decoder
